...
public class UserSyncAppTestVlad {
  ...
  public void test() {
    ...
    UserSyncApp.main("--operation", "copytofile1", "--csv", "src/test/resources/fisier.csv", "--csvWrite","target/users-all-vlad.csv");
    // adauge noii user in users-all
    UserSyncApp.main("--operation", "copytofile2", "--csv", "src/test/resources/fisier2.csv", "--csvWrite", "target/users-all-vlad.csv");
    // copieze toti userii din fisier.csv in users-all
