...
=======
  ...
  public List<TheUser> generateUsers(int n) {
    ...
    Faker faker = new Faker();
    for (int i = 0; i < n; i++) {
      TheUser user = new UserBuilder().setUsername(faker.name().username()).setFirstName(faker.name().firstName())
          .setLastName(faker.name().lastName()).setEmail(faker.name().username() + "@gmail.com").builder();
      theUserList.add(user);
