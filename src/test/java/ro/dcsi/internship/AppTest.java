...
public class AppTest {
  ...
  public void testHeaderDefinesTheColumn() {
    ...
    Assert.assertEquals("Ambrose", app.load(resources + "file-header2.csv").get(2).getLastname());
  }
  ...
  public void testAppIulian() {
    ...
    Assert.assertEquals(2, app.load(target + "file1").size());
    Assert.assertEquals(1, app.load(target + "file2").size());
    Assert.assertEquals("2", app.load(target + "file2").get(0).username);
  }

  @Test
  public void testLiviu() {
    BeanBasedUserDao controller = new BeanBasedUserDao();
    List<User> existingUsers = controller.load(resources + "users.csv");
    ...
    List<User> tempUsers = controller.load(target + "tempUsers.csv");
  ...
  public void testReadAppSorin() {
    ...
    System.out.println(appS.load(resources + "sorinUsersStyle.csv"));
  }

  public int getSize(String file) {
    return new BeanBasedUserDao().load(file).size();
  ...
  public void testGrigore() {
    ...
    List<User> ls = app.load(target + "file1");
