...
public class UserSyncApp {
  public static void main(String[] args) {
    ...
    Option destType = Option.builder("dt").longOpt("destination-type").hasArgs().argName("type")
        .desc("Type of the destination. Can be \"csv\" (CSV file) or \"fr\" (ForgeRock database").required().build();
    ...
    options.addOption(srcType).addOption(srcServer).addOption(srcUser).addOption(srcPass).addOption(srcFile)
        .addOption(destType).addOption(destServer).addOption(destUser).addOption(destPass).addOption(destFile);
    ...
    try {
      ...
      if (line.getOptionValue("st").equals("csv")) {
        ...
        if (!line.hasOption("ss") || !line.hasOption("su") || !line.hasOption("sp")) {
          ...
          src = new ForgeRockUserDao(
              new OpenIdConfig(line.getOptionValue("ss"), line.getOptionValue("su"), line.getOptionValue("sp")));
        }
      }
      if (line.getOptionValue("dt").equals("csv")) {
        ...
        if (!line.hasOption("ds") || !line.hasOption("du") || !line.hasOption("dp")) {
          System.out.println("You must specify a destination server address, username and password.");
        } else {
          dest = new ForgeRockUserDao(
              new OpenIdConfig(line.getOptionValue("ds"), line.getOptionValue("du"), line.getOptionValue("dp")));
        }
      }
